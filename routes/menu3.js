...
router.get('/', (req, res, next) => {
    dbCon.query('SELECT * FROM orderlist WHERE table_num=3 ORDER BY id asc', (err, rows) => {
        if (err) {
            req.flash('error', err);
            res.render('orderMenu/table3', { data: '' });
        } else {
            res.render('orderMenu/table3', { data: rows });
        }
    })
})

// display add menu page
router.get('/add3', (req, res, next) => {
    res.render('orderMenu/table3/add3', {
        name: '',
        status: '',
        price: 0,
        img: '',
...
router.get('/edit/(:id)', (req, res, next) => {
    ...
    dbCon.query('SELECT * FROM orderlist WHERE id = ' + id, (err, rows, fields) => {
        if (rows.length <= 0) {
            ...
            res.render('orderMenu/table3/edit', {
                title: 'Edit Menu',
                id: rows[0].id,
                name: rows[0].name,
